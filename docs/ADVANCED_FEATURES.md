...
### 5. Claude Code AI 代理 & 輸出風格整合
**來源**: 您現有的 `.claude` 配置
**功能**:
- 自動複製您的 4 個自訂 AI 代理到每個新專案
- 根據專案類型智能推薦輸出風格
- 專案特定的 Claude Code 配置優化
- 完整的權限和環境設定
...
**智能風格推薦**:
```bash
# API/API Gateway 專案 → 風格2 (正式風格)
AI 模型：Sonnet 4 │ 工作目錄：my-api │ 輸出風格：繁體中文 │ 版本：v1.0

# Web 應用專案 → 風格3 (符號風格)  
🤖 Sonnet 4 ┃ 📁 my-webapp ┃ 🎨 繁體中文

# 微服務專案 → 風格5 (技術風格含時間)
【v1.0】模型：Sonnet 4 │ 路徑：my-service │ 模式：繁體中文 │ 時間：14:30
```

**專案目錄結構**:
```
my-project/
├── .claude/                    # 專案專用 Claude Code 配置
│   ├── agents/
│   │   ├── code-reviewer.md
│   │   ├── error-debugger.md
│   │   ├── prd-writer.md
│   │   └── steering-architect.md
│   ├── statusline-tc.sh        # 優化的狀態列腳本
│   └── settings.json           # 專案特定設定
├── docs/
│   ├── claude-config.md        # Claude Code 配置說明
│   └── ai-prompts.md          # AI 提示範本
└── CLAUDE.md                   # 開發指南
```
