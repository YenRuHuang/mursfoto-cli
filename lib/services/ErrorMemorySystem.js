...
class ErrorMemorySystem {
  ...
  async recordError(errorInfo) {
    try {
      // 參數驗證 - 確保 errorInfo 存在
      if (!errorInfo) {
        logger.error('錯誤記錄失敗: errorInfo 參數未提供');
        return null;
      }
      ...
      // 安全處理錯誤對象
      const safeError = this.normalizeError(errorInfo.error);
  ...
  generateErrorFingerprint(error, command, context) {
    const key = `${command}|${error.message}|${error.code}|${JSON.stringify(context)}`;
