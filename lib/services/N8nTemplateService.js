...
app.post('/webhook/:workflowId', async (req, res) => {
  try {
    ...
    this.logger?.info(\`📨 收到 webhook 請求: \${workflowId}\`, payload);
...
app.listen(port, () => {
  this.logger?.info(\`🚀 \${process.env.npm_package_name || 'n8n-service'} 運行於 http://localhost:\${port}\`);
  this.logger?.info(\`📋 模板: ${n8nTemplate.name}\`);
  this.logger?.info(\`🔗 Webhook: http://localhost:\${port}/webhook/[workflow-id]\`);
});
