...
class GUIServer {
  ...
  setupSocketHandlers() {
    this.io.on('connection', (socket) => {
      console.log(chalk.blue('🔗 GUI 客戶端已連接'))
      ...
      socket.on('disconnect', () => {
        console.log(chalk.yellow('🔌 GUI 客戶端已斷線'))
      })
    })
  }
    ...
    return new Promise((resolve, reject) => {
      this.server.listen(this.port, this.host, (err) => {
        if (err) {
          ...
          console.log(chalk.cyan(`🌐 訪問: http://${this.host}:${this.port}`))
  ...
  async stop() {
    return new Promise((resolve) => {
      this.server.close(() => {
        console.log(chalk.yellow('🛑 GUI 服務器已停止'))
        resolve()
      })
    })
  }
