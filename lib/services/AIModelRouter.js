...
class AIModelRouter {
  ...
  async generate (prompt, options = {}) {
    ...
    try {
      // 策略 1: 優先嘗試 Claude Code Provider (Cline IDE)
      if (this.claudeCodeProvider !== 'disabled' && await this.isClineApiHealthy()) {
        try {
      ...
      // 策略 2: 嘗試本地模型（如果 Claude Code 不可用或被禁用）
    ...
    // 策略 3: 最後備援使用原生 Claude API
    if (!result) {
      if (!this.claudeApiKey) {
        throw new Error('所有 AI 服務都不可用：請配置 Claude Code Provider、本地模型或 Anthropic API Key')
      }
