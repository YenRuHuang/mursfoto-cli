...
/**
 * 🤖 AI 代碼生成器 - Claude Code 專用
 * 基於自然語言描述生成完整的程式碼，專門使用 Claude Code
 */
class AICodeGenerator {
  constructor() {
    this.apiKey = process.env.ANTHROPIC_API_KEY;
    if (!this.apiKey) {
      console.warn('⚠️ 警告：未找到 ANTHROPIC_API_KEY，請設定環境變數');
    }
    this.claudeModel = 'claude-sonnet-4-20250514'; // 使用 Sonnet-4 最新模型
    // 將模板初始化延遲到方法定義之後
  ...
  /**
   * 📊 需求分析 - 建議透過 Cline (Claude Code) 進行深度分析
   * 本功能提供基礎分析，真正的 AI 輔助開發建議使用 Cline
   */
  async analyzeRequirements(description, type) {
    console.log('💡 提示：建議使用 Cline (Claude Code) 進行深度 AI 分析和代碼生成');
    console.log('📝 CLI 工具專注於自動化任務，AI 輔助開發請使用 Cline');
    
    // 優先使用本地分析，避免重複的 API 調用
    return this.fallbackAnalysis(description, type);
    
    // 如果用戶真的需要 API 調用（罕見情況），才使用以下代碼
    if (this.apiKey && process.env.FORCE_API_CALL === 'true') {
      try {
        const claudeAnalysis = await this.callClaudeAPI({
          system: `您是專業的軟體架構師，專門分析用戶需求並提供技術建議。請分析以下需求並以 JSON 格式回應：
{
  "entities": ["識別的實體列表"],
  "operations": ["需要的操作列表"],
  "requirements": ["功能需求列表"],
  ...
  "technologies": ["建議使用的技術"]
          ...
          messages: [
            {
              role: "user",
              content: `請分析這個${type}專案需求：${description}`
            }
          ]
        });

        return JSON.parse(claudeAnalysis);
      } catch (error) {
        console.warn('Claude API 調用失敗，使用本地分析:', error.message);
        return this.fallbackAnalysis(description, type);
      }
    }
  }
