...
class IntelligentLearningSystem extends EventEmitter {
  ...
  async init () {
    try {
      // 快速初始化，避免阻塞命令執行
      if (process.env.NODE_ENV === 'test' || process.env.MURSFOTO_QUICK_MODE === 'true') {
        // 測試模式下快速初始化
        this.quickInit()
        return
      }
  ...
  async recordCommand (commandData) {
    // 快速模式下跳過複雜操作
    if (process.env.MURSFOTO_QUICK_MODE === 'true' || !this.initialized) {
      return
    }
    ...
    try {
      ...
      // 即時學習和決策更新（非阻塞）
      setImmediate(() => this.updateDecisionModel(command, success, context))
    } catch (error) {
      // 忽略記錄錯誤，不影響命令執行
    }
  }
  ...
  updateLearningConfidence () {
    ...
    if (commands.length === 0) {
      this.decisionModel.learningConfidence = 0.0
      return
    }
    ...
    // 基於命令數量的基礎置信度
    let confidence = Math.min(totalCommands / 20, 0.7) // 最高 70% 基礎置信度
    ...
    // 多樣性加成（不同類型命令的使用）
    const diversityBonus = Math.min(commands.length / 10, 0.2)
    confidence += diversityBonus

    // 確保置信度在合理範圍內
    confidence = Math.max(0.0, Math.min(confidence, 0.95))
  ...
  getLearningStatistics () {
    ...
    if (commands.length === 0) {
      return {
        ...
        learningConfidence: 0, // 沒有學習數據時置信度為 0%
        workflowPatterns: 0,
        sessionCommands: 0,
