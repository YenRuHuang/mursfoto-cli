...
class IntelligentLearningSystem extends EventEmitter {
  ...
  updateLearningConfidence() {
    const totalCommands = Object.values(this.userBehavior.commands)
      .reduce((sum, cmd) => sum + cmd.count, 0);
    
    // 基於數據量和成功率計算置信度
    let confidence = Math.min(totalCommands / 100, 1.0); // 基礎置信度
    
    const averageSuccessRate = Object.values(this.userBehavior.commands)
      .reduce((sum, cmd) => sum + cmd.successRate, 0) / 
      Object.keys(this.userBehavior.commands).length;
    
    confidence *= averageSuccessRate; // 調整成功率權重
  ...
  getLearningStatistics() {
    ...
    const averageSuccessRate = Object.values(this.userBehavior.commands)
      .reduce((sum, cmd, _, arr) => sum + cmd.successRate / arr.length, 0);
    
    return {
      totalCommands,
      uniqueCommands: Object.keys(this.userBehavior.commands).length,
      averageSuccessRate: Math.round(averageSuccessRate * 100) / 100,
      learningConfidence: this.decisionModel.learningConfidence,
      ...
      mostUsedCommands: Object.entries(this.userBehavior.commands)
        .sort(([,a], [,b]) => b.count - a.count)
        .slice(0, 5)
        .map(([cmd, data]) => ({ command: cmd, count: data.count, successRate: data.successRate }))
    };
  }
